...
pub enum DepSource {
    Local(String),
    Registry(String),
    /// Bare dep name, resolved from the mx install prefix (~/.mx/lib/<name>/)
    Installed,
}

/// Return the mx install prefix lib directory.
/// Uses MX_HOME env var if set, otherwise defaults to ~/.mx/lib.
fn mx_lib_dir() -> Option<PathBuf> {
    std::env::var_os(crate::identity::ENV_HOME)
        .map(|h| PathBuf::from(h).join("lib"))
        .or_else(|| std::env::var_os("HOME").map(|h| PathBuf::from(h).join(crate::identity::HOME_DIR).join("lib")))
}

/// Resolve an installed dep name to its path under the install prefix.
fn resolve_installed_dep(name: &str) -> Option<PathBuf> {
    let lib_dir = mx_lib_dir()?;
...
";
    ...
    fn test_parse_lockfile() {
        let content = "\
# m2.lock - generated by mxpkg resolve
